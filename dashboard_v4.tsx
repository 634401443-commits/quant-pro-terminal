...
function generateDrivingLogic(card: IndustryCausalCard): string {
  ...
  if (avgChange < -2) {
    return `板块跌幅${avgChange.toFixed(1)}%，${card.weakestStock}领跌${card.weakestChange.toFixed(1)}%，资金流出`;
  }
...
function buildCausalChain(stocks: any[], industryCards: IndustryCausalCard[]): CausalChain | null {
  ...
  if (anomalies.length === 0 && industryCards.length === 0) return null;

  let target: any = null;
  let eventText = '';
  let evidenceCount = 0;

  if (industryCards.length > 0 && industryCards[0].change > 2) {
    const topCard = industryCards[0];
    eventText = topCard.drivingLogic;
    evidenceCount = 1;
    const indAnomalies = anomalies.filter(s => s.industry === topCard.name);
    if (indAnomalies.length > 0) {
      indAnomalies.sort((a, b) => Math.abs(b.change || 0) - Math.abs(a.change || 0));
      target = indAnomalies[0];
    } else if (anomalies.length > 0) {
      anomalies.sort((a, b) => Math.abs(b.change || 0) - Math.abs(a.change || 0));
      target = anomalies[0];
    } else {
      const sameInd = stocks.filter(s => s.industry === topCard.name);
      if (sameInd.length > 0) target = [...sameInd].sort((a, b) => (b.change || 0) - (a.change || 0))[0];
    }
  } else if (anomalies.length > 0) {
    anomalies.sort((a, b) => Math.abs(b.change || 0) - Math.abs(a.change || 0));
    target = anomalies[0];
  ...
  const fundFlow = isUp
    ? (target.turnover || 0) > 0 ? `成交活跃，主力资金偏多` : '资金偏多'
    : (target.turnover || 0) > 0 ? `成交萎缩，资金偏空` : '资金偏空';
  evidenceCount++;

  const marketPerf = `${target.name} ${isUp ? '+' : ''}${changePct}%`;
  evidenceCount++;

  const directionConsistent = true;
...
export default function DashboardPage() {
  ...
  // 行业卡片：涨幅前3 + 跌幅第1
  const top3Industries = industryCards.slice(0, 3);
  const bottom1Industry = industryCards.length > 3 ? industryCards[industryCards.length - 1] : null;
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <div className="text-lg font-bold" style={{ color: factorStrategy.avgChange >= 0 ? RISE_COLOR : FALL_COLOR }}>
                {factorStrategy.avgChange >= 0 ? '+' : ''}{factorStrategy.avgChange.toFixed(1)}%
            ...
            <div className="space-y-2">
              ...
              <div className="text-lg font-bold" style={{ color: etfStrategy.avgChange >= 0 ? RISE_COLOR : FALL_COLOR }}>
                {etfStrategy.avgChange >= 0 ? '+' : ''}{etfStrategy.avgChange.toFixed(1)}%
                {factorWin && <span className="ml-1 text-success">✅</span>}
              </div>
              <div className="text-[10px] text-muted-foreground">持仓Top3</div>
        ...
        <div className="rounded-lg border border-border/40 bg-card/40 p-3">
          ...
          <div className="pt-2 border-t border-border/20">
            ...
            {top3Industries.length > 0 ? (
              ...
              <div className="text-[10px] text-muted-foreground text-center py-3">暂无行业数据</div>
            )}
          </div>
        </div>
