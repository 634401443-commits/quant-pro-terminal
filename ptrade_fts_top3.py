# -*- coding: utf-8 -*-
"""
PTrade ETF 轮动 Top3 — FTS 精英因子版（利用用户 v3.1-A 量化选股策略）
=====================================================================
选股: 5 个 FTS 因子等权打分(0-100) —— 移植自 v3.1-A（已跑通回测）
轮动: 按 FTS 分取 Top3；持仓掉出 Top3 → 卖出；Top3 空仓且分>=门槛 → 买入
环境约束（LogEngine 实测）:
...
def initialize(context):
    # 交易标的池（用户指定）：行业10 + 宽基2 + 跨境5 + 商品3
    g.etf_pool = [
        # 行业10只（T+1）
        '512010.SS', '512690.SS', '512480.SS', '515880.SS',
        '512760.SS', '159928.SZ', '512880.SS', '512800.SS',
        '512660.SS', '515030.SS',
        # 宽基2只（T+1）
        '588000.SS', '159352.SZ',
        # 跨境5只（T+0）
        '513100.SS', '513050.SS', '513500.SS', '513520.SS', '513030.SS',
        # 商品3只（T+0：油气159518 + 黄金159934 + 有色159980）
        '159518.SZ', '159934.SZ', '159980.SZ'
    ]
    g.top_n = 3
    g.sell_rank = 6            # 缓冲带：掉出前6名才卖（降换手）
    g.cooldown_days = 5        # 卖出后冷却天数（期间不买回，降换手）
    g.vol_per = 10000          # 建议股数
    g.max_amount = 50000       # 单笔金额上限
    g.min_score = 55           # v4优化：买入门槛 32→55（过滤弱信号，五年弱信号多拉低夏普）
    g.last_date = None
    g.cooldown = {}            # code -> 卖出日期str
    # T+0 标的（跨境5 + 商品3 当天可买卖；行业/宽基 A股 ETF 为 T+1）
    g.t0 = {'513100.SS', '513050.SS', '513500.SS', '513520.SS', '513030.SS',
            '159518.SZ', '159934.SZ', '159980.SZ'}

    # ===== v2新增：风控参数（统一在此调整）=====
    g.stop_loss_pct = 8            # P0-1 单票硬止损：浮亏超过该%立即市价清仓
    ...
    g.take_profit_tiers = [(20, 0.3), (30, 0.3), (40, 1.0)]  # v8优化：第一档15→20（让利润奔跑，追踪止盈优先兜底）
    ...
    # ===== v8优化：动态凯利参数（按市场趋势切换口径）=====
    # bull:  短期牛市口径 58%/1.90 → 满凯利≈36.8%（激进）
    # neutral: 五年全口径 49%/1.20 → 满凯利≈6.7%（稳健）
    # bear:  熊市口径 40%/1.0 → 满凯利≈0%（防御，仅靠 trend_mod 兜底极小仓位）
    g.kelly_params = {
        'bull':    {'win_rate': 0.58, 'avg_win': 1.90},
        'neutral': {'win_rate': 0.49, 'avg_win': 1.20},
        'bear':    {'win_rate': 0.40, 'avg_win': 1.00},
    }
    g.kelly_fraction = 1.0     # 全凯利（动态参数已按趋势调仓，不再需要半凯利压缩）

    log.info("=" * 56)
    log.info("ETF轮动Top3 FTS因子版 (5因子等权) 标的:%d Top%d" % (len(g.etf_pool), g.top_n))
...
def dynamic_scale(drawdown_pct):
    """P2-6 动态仓位系数：回撤0-5%满仓 / 5-10%×0.7 / 10-15%×0.5 / >15% 0（不开新仓）"""
    if drawdown_pct <= 5:
        return 1.0
    if drawdown_pct <= 10:
        return 0.7
...
def calc_kelly_position(score, price, total_value, drawdown_pct, market_trend):
    """v8优化：动态凯利参数 + FTS评分/账户回撤/市场趋势 三重修正
    核心改进：
      - win_rate/avg_win 按 market_trend 动态切换（bull/neutral/bear 三套口径）
      - 仓位下限 max(3000, total_value*0.005)：适配大资金（0.5%起步）同时保小账户
    返回 (建议金额, 综合比例, 明细dict)
    """
    # 1. 动态凯利参数（按市场趋势切换）
    params = g.kelly_params.get(market_trend, g.kelly_params['neutral'])
    win_rate = params['win_rate']
    avg_win = params['avg_win']
    full_kelly = (win_rate * avg_win - (1 - win_rate)) / avg_win
    full_kelly = max(0, min(0.5, full_kelly))
    # 2. 凯利分数
    base_kelly = full_kelly * g.kelly_fraction
    # 3. FTS 评分修正
    if score >= 80:
        score_mod = 1.2
    elif score >= 70:
    ...
    if market_trend == 'bull':
        ...
        trend_mod = 0.4
    # 6. 综合比例（上限30%）
    final_ratio = base_kelly * score_mod * drawdown_mod * trend_mod
    final_ratio = max(0, min(0.30, final_ratio))
    # 7. 金额
    amount = total_value * final_ratio
    # 8. 上下限（v8优化：下限 max(3000, 0.5%)，平衡大资金灵活性与小账户门槛）
    min_amount = max(3000, total_value * 0.005)
    max_amount = min(80000, total_value * 0.25)
    detail = {
        'win_rate': win_rate, 'avg_win': avg_win,
        'full_kelly': full_kelly, 'base_kelly': base_kelly,
        'score_mod': score_mod, 'drawdown_mod': drawdown_mod,
        'trend_mod': trend_mod, 'final_ratio': final_ratio, 'amount': amount,
    }
    if amount < min_amount:
        detail['reason'] = '金额%.0f低于下限%.0f' % (amount, min_amount)
        return 0, final_ratio, detail
    if amount > max_amount:
        amount = max_amount
        detail['amount'] = amount
    return amount, final_ratio, detail
...
def handle_data(context, data):
    try:
        ...
        if drawdown >= g.max_drawdown_halve and not g.halved:
        ...
        elif drawdown >= g.max_drawdown_pause:
        ...
        buy_allowed = (not g.drawdown_mode) and market != 'bear'
        ...
        for code, pos in list(context.portfolio.positions.items()):
            ...
            # v8优化：熊市持仓风控（熊市 + 浮亏>5% 或 连续3日跌破MA20 → 清仓）
            if market == 'bear':
        ...
        # 5) v2/v3：买入（风控闸门 + 凯利动态仓位）
        if not buy_allowed:
            if g.last_buy_allowed:
                log.info("【风控】%s %s，跳过开仓" % (
                    today_str, '熊市不开仓' if market == 'bear' else '账户回撤暂停开仓'))
            ...
            for code, score, price in scored[:g.top_n]:
                ...
                try:
                    ...
                    pass
                amount, final_ratio, detail = calc_kelly_position(
                    score, price, total_value, drawdown, market)
                if amount <= 0:
                    skip_reasons.append("%s(凯利跳过)" % code)
                    continue
                ...
                log.info("【买入】%s %d股 @%.3f 金额%.0f | FTS分%.0f 凯利%.1f%%(胜率%.0f%%/盈亏比%.1f)×评分%.1f×回撤%.1f×趋势%.1f=%.1f%%" % (
                    code, vol, price, price * vol, score,
                    detail['base_kelly'] * 100,
                    detail['win_rate'] * 100,
                    detail['avg_win'],
                    detail['score_mod'],
                    detail['drawdown_mod'],
                    detail['trend_mod'],
                    detail['final_ratio'] * 100))
            if skip_reasons:
