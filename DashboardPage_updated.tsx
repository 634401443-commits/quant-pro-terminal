...
import {
  ...
  Clock, BarChart3, Flame, ChevronRight, Copy, Check
} from 'lucide-react';
...
function calcScoreTrend(scoreKey: string, currentScore: number): { trend: 'up' | 'down' | 'flat'; diff: number } {
  try {
    ...
    // 尝试获取前一天的评分，如果当天没有，则找最近的历史记录
    let prevScore: number | null = null;
    if (history[yesterdayKey] != null) {
      prevScore = history[yesterdayKey];
    } else {
      // 找最近的非今日记录
...
function calcEnvMatch(
  ...
  if (isFactor) {
    ...
    const matched = top3.filter(h => {
      const cat = h.category || '';
      const catChange = etfCategoryChanges[cat] || 0;
      return (cat === '行业' || cat === '主题') && catChange > 0;
    });
    return Math.round((matched.length / top3.length) * 100);
  }
}
...
function get5DayReturnText(holdings: any[]): string {
  ...
  const dayLabels = ['4天前', '3天前', '2天前', '昨天', '今天'];
  return days.map((v, i) => {
    return dayLabels[i] + ': ' + (v >= 0 ? '+' : '') + v + '%';
  }).join(' | ');
}
...
function copyHoldingsToSimulation(holdings: any[], type: 'factor' | 'etf'): string {
  try {
    ...
    const names = holdings.slice(0, 5).map(h => h.name).filter(Boolean);
    const data = {
      type,
      codes,
      names,
...
export default function DashboardPage() {
  ...
  const [copyFeedback, setCopyFeedback] = useState<{ msg: string; show: boolean }>({ msg: '', show: false });

  const handleCopyFactor = () => {
    const msg = copyHoldingsToSimulation(factorStrategy.holdings, 'factor');
    setCopyFeedback({ msg, show: true });
    setTimeout(() => setCopyFeedback({ msg: '', show: false }), 2000);
  };
  ...
  const factorScoreTrend = useMemo(() =>
    calcScoreTrend('factor', factorQuality.score),
  [factorQuality.score]);

  const etfScoreTrend = useMemo(() =>
    calcScoreTrend('etf', etfQuality.score),
  [etfQuality.score]);

  // ============ 调整2: 环境匹配度计算 ============
  const factorEnvMatch = useMemo(() =>
    calcEnvMatch(true, factorStrategy.holdings, dominantFactor, etfCategoryChanges),
  [factorStrategy.holdings, dominantFactor, etfCategoryChanges]);

  const etfEnvMatch = useMemo(() =>
    calcEnvMatch(false, etfStrategy.holdings, dominantFactor, etfCategoryChanges),
  [etfStrategy.holdings, dominantFactor, etfCategoryChanges]);

  const factorMatchLabel = useMemo(() => getMatchLabel(factorEnvMatch), [factorEnvMatch]);
  const etfMatchLabel = useMemo(() => getMatchLabel(etfEnvMatch), [etfEnvMatch]);
  ...
  // 渲染趋势标识
  const renderTrend = (trendData: { trend: string; diff: number }) => {
    if (trendData.trend === 'up') {
      return <span className="ml-1 text-[10px]" style={{ color: RISE_COLOR }}>↑{trendData.diff}</span>;
    }
    if (trendData.trend === 'down') {
      return <span className="ml-1 text-[10px]" style={{ color: FALL_COLOR }}>↓{Math.abs(trendData.diff)}</span>;
    }
        ...
        <div className="rounded-lg border border-border/40 bg-card/40 p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              {/* 展开持仓明细 */}
              <FactorPositionExpand holdings={factorStrategy.holdings} />
              <div className="text-[10px] text-muted-foreground pt-1 border-t border-border/20">{factorAttributionBreakdown}</div>
              {/* 调整3: 复制持仓按钮 */}
              <div className="flex items-center gap-2 pt-1">
                ...
                {copyFeedback.show && (
                  <span className="text-[9px] flex items-center gap-0.5" style={{ color: '#22c55e' }}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  ...
                  {showEtfTrend && (
                    <div className="absolute top-full left-0 mt-1 z-10 w-[280px] p-2 rounded-lg border border-border/40 bg-card shadow-lg text-[10px]">
                      <div className="font-medium text-foreground mb-1">ETF轮动近5日收益趋势</div>
              ...
              {/* 展开持仓明细 */}
              <EtfPositionExpand holdings={etfStrategy.holdings} />
              <div className="text-[10px] text-muted-foreground pt-1 border-t border-border/20">{etfAttributionBreakdown}</div>
              {/* 调整3: 复制持仓按钮 */}
              <div className="flex items-center gap-2 pt-1">
                <button
                  className="text-[9px] text-primary hover:underline flex items-center gap-0.5"
