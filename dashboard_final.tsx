...
export default function DashboardPage() {
  ...
  // ---- 指数走势对比图 ----
  const indexTrendOption = useMemo<EChartsOption>(() => {
    const idx300 = indices.find(i => i.code === '000300') || indices[0];
    const idx1 = indices.find(i => i.code === '000001') || indices[1];
    const kline = idx300?.kline || [];
    const days = kline.length;
    if (days === 0) return { series: [] } as EChartsOption;
    return {
      tooltip: { trigger: 'axis' },
      legend: { type: 'scroll', bottom: 0, itemWidth: 12, itemHeight: 8, textStyle: { fontSize: 10 } },
      grid: { left: '3%', right: '3%', bottom: '15%', top: '5%', containLabel: true },
      xAxis: {
        type: 'category',
        data: Array.from({ length: days }, (_, i) => `D${i + 1}`),
        axisLabel: { fontSize: 10, interval: Math.max(1, Math.floor(days / 6)) },
      },
      yAxis: { type: 'value', axisLabel: { fontSize: 10 } },
      series: [
        {
          name: idx300?.name || '沪深300',
          type: 'line',
          data: kline,
          smooth: true,
          symbol: 'none',
        ...
        },
        {
          ...
          smooth: true,
          symbol: 'none',
          lineStyle: { width: 1.5, color: CHART_COLORS[1] },
        },
      ],
    };
  }, [indices]);
  ...
  const industryBarOption = useMemo<EChartsOption>(() => {
    const industryMap = new Map<string, { changes: number[]; count: number }>();
    for (const st of realStocks) {
      const ind = st.industry || '其他';
      if (!industryMap.has(ind)) industryMap.set(ind, { changes: [], count: 0 });
      const d = industryMap.get(ind)!;
      d.changes.push(st.change || 0);
      d.count++;
    }
    const topN = Array.from(industryMap.entries())
      .map(([name, data]) => ({
        name,
        change: Number((data.changes.reduce((a, b) => a + b, 0) / data.changes.length).toFixed(2)),
      }))
    ...
    return {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `${p.name}<br/>涨跌幅: ${Number(p.value) > 0 ? '+' : ''}${p.value}%`;
        },
      },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-lg border border-border/40 bg-card/40 p-3">
          ...
          <div className="mt-2 pt-2 border-t border-border/20">
            ...
            <ReactECharts option={industryBarOption} theme="ud" style={{ height: '180px' }} />
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-border/40 bg-card/40 p-3">
        ...
        <ReactECharts option={indexTrendOption} theme="ud" style={{ height: '260px' }} />
      </div>
