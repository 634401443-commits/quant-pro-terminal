# -*- coding: utf-8 -*-
"""
PTrade ETF 轮动 Top3 混合因子版 — v3.1-A × QUANT PRO（2026-08 回测优选）
=====================================================================
选股: 混合因子打分(0-100) = 50% v3.1-A五因子 + 50% QUANT PRO五因子
轮动: 按分取 Top3；持仓掉出前8名 → 卖出；空仓且分>=45 → 买入（等权）
回测: 牛市区间(核心10池) 收益37.0%/回撤10.1%/夏普1.10
      长区间(8只池)      收益30.0%/回撤15.6%（较纯v3.1-A版回撤↓10个点）
环境约束（LogEngine 实测）:
...
def initialize(context):
    ...
    # ===== P1.2: 从 trade_signal.json 加载统一因子配置 =====
    g.factor_cfg = _load_signal_factor_config()
    _br = g.factor_cfg.get('blend_ratio', {})
    log.info("因子配置: blend v31a=%.2f qp=%.2f" % (
        _br.get('v31a', 0.5), _br.get('qp', 0.5)))


def _load_signal_factor_config():
    """P1.2: 从 trade_signal.json 读取统一因子配置（PTrade研究目录）"""
    import json as _json
    default = {
        "blend_ratio": {"v31a": 0.50, "qp": 0.50},
        "v31a_weights": [0.20, 0.20, 0.20, 0.20, 0.20],
        "qp_weights": {"xsmom": 0.25, "lowvol": 0.20, "vpcorr": 0.20, "trend": 0.20, "bias": 0.15},
    }
    for path in ['/home/fly/notebook/trade_signal.json', 'trade_signal.json']:
        try:
            with open(path, 'r') as f:
                data = _json.load(f)
            cfg = data.get('factor_config', {})
            if not cfg:
                continue
            result = dict(default)
            result["blend_ratio"] = cfg.get("blend_ratio", default["blend_ratio"])
            qp_group = cfg.get("factor_groups", {}).get("qp", {})
            for fac in qp_group.get("factors", []):
                result["qp_weights"][fac["name"]] = fac["weight"]
            v31a_group = cfg.get("factor_groups", {}).get("v31a", {})
            v31a_factors = v31a_group.get("factors", [])
            if v31a_factors:
                result["v31a_weights"] = [f["weight"] for f in v31a_factors]
            log.info("统一因子配置已从 %s 加载" % path)
            return result
        except Exception:
            continue
    log.info("trade_signal.json 未找到，使用默认因子配置")
    return default
...
FACTOR_W = [0.20, 0.20, 0.20, 0.20, 0.20]  # 默认等权（P1.2: 运行时从g.factor_cfg覆盖）


def compute_fts_score(df):
    """混合因子综合打分 0-100 = 50% v3.1-A五因子 + 50% QUANT PRO五因子
    v3.1-A: 波动率回归|动量|五维动量|MA趋势|量价动量（等权）
    QUANT PRO: 超额动量|低波动|量价相关|趋势|乖离（25/20/20/20/15）
    P1.2: 权重从 factor_config.json 统一读取
    """
    try:
        ...
        n = len(c)
        # P1.2: 从统一配置读取权重
        _cfg = g.factor_cfg if hasattr(g, 'factor_cfg') else {}
        _fw = _cfg.get('v31a_weights', FACTOR_W)
        _br = _cfg.get('blend_ratio', {'v31a': 0.5, 'qp': 0.5})
        sigs = [
        ...
        scores = [(s + 1) * 10 * _fw[i] for i, s in enumerate(sigs)]
        v31 = sum(scores) * (100.0 / 20.0)
        qp = factor_qp_score(df)
        if qp is None:
            return v31
        _r_v31 = _br.get('v31a', 0.5)
        _r_qp = _br.get('qp', 0.5)
        return v31 * _r_v31 + qp * _r_qp
    except Exception:
        return None


def rolling_std(arr, w):
    """滚动标准差（QUANT PRO 因子用，纯 numpy.std 保守实现）"""
    n = len(arr); r = np.zeros(n)
    for i in range(n):
        s = max(0, i - w + 1)
        r[i] = float(np.std(arr[s:i + 1]))
    return r


def factor_qp_score(df):
    """QUANT PRO 版 FTS 五因子（xsmom/lowvol/vpcorr/trend/bias）0-100"""
    try:
        c = df['close'].values.flatten().astype(float)
        v = df['volume'].values.flatten().astype(float)
        n = len(c)
        ...
        xsmom = np.clip((sx + 1) * 50, 0, 100)
        # 低波动
        rstd = rolling_std(ret, 20)[-1] * np.sqrt(252)
        lowvol = np.clip(100 - rstd * 100 * 5, 0, 100)
        # 量价相关（10日排序）
        w = 10
        vi = v[-w:]; ci = c[-w:]
        vr = np.argsort(np.argsort(vi)).astype(float) / w
        cr = np.argsort(np.argsort(ci)).astype(float) / w
        cv = np.sum((vr - np.mean(vr)) * (cr - np.mean(cr))) / max(
            np.sqrt(np.sum((vr - np.mean(vr)) ** 2) * np.sum((cr - np.mean(cr)) ** 2)), 1e-10)
        vpcorr = np.clip((cv + 1) * 50, 0, 100)
        # 趋势（20日线性回归斜率+拟合优度）
        ...
        trend = np.clip((st + 1) * 50, 0, 100)
        # 乖离（相对 MA20，超跌高分）
        m20v = sma(c, 20)[-1]
        bias = -(c[-1] - m20v) / max(m20v, 1e-10)
        bias_s = np.clip((np.tanh(bias * 15) + 1) * 50, 0, 100)
        # P1.2: 权重从统一因子配置读取
        _qw = (g.factor_cfg if hasattr(g, 'factor_cfg') else {}).get(
            'qp_weights', {"xsmom": 0.25, "lowvol": 0.20, "vpcorr": 0.20, "trend": 0.20, "bias": 0.15})
        return float(np.clip(
            xsmom * _qw["xsmom"] + lowvol * _qw["lowvol"] +
            vpcorr * _qw["vpcorr"] + trend * _qw["trend"] +
            bias_s * _qw["bias"], 0, 100))
    except Exception:
        return None
...
def _try_fetch_remote_signal():
    """P2.1: 尝试通过HTTP从ptrade_bridge拉取最新信号（消除手动上传断点）
    成功→写入本地trade_signal.json；失败→静默回退到本地文件
    """
    try:
        try:
        ...
        _url = 'http://127.0.0.1:8766/api/signal/latest'
        _resp = urlopen(_url, timeout=3)
        import json as _json
        _data = _json.loads(_resp.read().decode('utf-8'))
        if _data.get('code') == 1 and _data.get('data'):
            _payload = _data['data']
            _path = '/home/fly/notebook/trade_signal.json'
            with open(_path, 'w') as f:
                _json.dump(_payload, f)
            log.info("P2.1: 信号已从bridge自动拉取(%s)" % _payload.get('timestamp', '?'))
            # 同步更新因子配置
            if _payload.get('factor_config'):
                g.factor_cfg = _load_signal_factor_config()
            return True
    except Exception as e:
        log.info("P2.1: bridge拉取跳过(使用本地文件): %s" % str(e)[:80])
    return False
...
def calc_atr_pct(code):
    """P2.2: 计算20日ATR百分比（ATR/价格*100），用于动态止盈阈值"""
    if code in _ATR_CACHE:
        return _ATR_CACHE[code]
    try:
        df = get_ohlc(code, 25)
        ...
        _ATR_CACHE[code] = atr_pct
        return atr_pct
    except Exception:
        return 2.0

def get_dynamic_trailing(code):
    """P2.2: 根据ATR%动态返回 (activate, drawback) 止盈阈值
    低波动(ATR%<1.5): activate=3, drawback=3-4
    中波动(1.5-3):   activate=5, drawback=5-7
    高波动(>3):       activate=8, drawback=8-10
    """
    atr_pct = calc_atr_pct(code)
    activate = max(3.0, min(8.0, atr_pct * 2.0))
    drawback = max(3.0, min(10.0, atr_pct * 2.5))
    return activate, drawback
...
def handle_data(context, data):
    try:
        ...
        # P2.1: 每日开盘前尝试HTTP拉取最新信号（消除手动上传断点）
        _try_fetch_remote_signal()
        # P2.2: 每日清理ATR缓存（确保波动率数据为最新）
        _ATR_CACHE.clear()
        ...
        for code, pos in list(context.portfolio.positions.items()):
            ...
            # P2.2: 波动率自适应止盈 — ATR动态阈值替换固定5%
            trailing_triggered = False
            _act, _draw = get_dynamic_trailing(code)
            if profit_pct >= _act and hi > 0:
                dd = (hi - cp) / hi * 100
                if dd >= _draw:
                    order(code, -sell_left)
                    _record_sell_stat(sell_left, cp, cost)
                    ...
                    g.highest_prices.pop(code, None)
                    g.tp_taken.pop(code, None)
                    log.info("【止盈】%s(%s) 追踪清仓 %d股 @%.3f 最高%.3f回撤%.1f%%(阈值%.1f%%)" % (
                        code, tag, sell_left, cp, hi, dd, _draw))
...
def after_trading_end(context, data):
    ...
    try:
        ...
        # P3.1: 回传持仓快照
        _report_positions(context)
    except Exception as e:
        log.info("after_trading_end异常: %s" % e)


def _report_positions(context):
    """P3.1: 生成持仓快照并推送到bridge服务（失败时写本地文件）"""
    try:
        import json as _json
        positions = []
        for code, pos in list(context.portfolio.positions.items()):
            ...
            if amt <= 0:
                continue
            cost = float(getattr(pos, 'cost_basis', 0) or getattr(pos, 'avg_cost', 0) or 0)
            cp = get_current_price(code)
            if cp is None:
                cp = cost
            pnl = (cp - cost) * amt if cost > 0 else 0
            positions.append({
                'code': code, 'amount': amt, 'cost': round(cost, 4),
                'price': round(cp, 4), 'pnl': round(pnl, 2),
                'pnl_pct': round((cp / cost - 1) * 100, 2) if cost > 0 else 0,
            })
        payload = {
            'timestamp': str(context.current_dt),
            'total_value': float(getattr(context.portfolio, 'total_value', 0) or 0),
            'positions': positions,
            'stat_buy_count': g.stat_buy_count,
            'stat_sell_count': g.stat_sell_count,
            'stat_realized_pnl': round(g.stat_realized_pnl, 2),
        }
        # 写本地文件
        try:
            with open('/home/fly/notebook/position_snapshot.json', 'w') as f:
        ...
        try:
            ...
            _body = _json.dumps(payload).encode('utf-8')
            _req = Request('http://127.0.0.1:8766/api/positions/report',
                          data=_body, headers={'Content-Type': 'application/json'})
            urlopen(_req, timeout=3)
            log.info("P3.1: 持仓快照已推送(%d只)" % len(positions))
        except Exception as e:
            log.info("P3.1: 持仓推送跳过(已写本地): %s" % str(e)[:60])
    except Exception as e:
        log.info("P3.1: 持仓回传异常: %s" % str(e)[:80])
